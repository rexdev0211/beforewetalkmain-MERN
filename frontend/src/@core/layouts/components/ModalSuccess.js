    ...
    <Modal isOpen={successModal} toggle={() => setSuccessModal(!successModal)} className='modal-dialog-centered checkEmail-modal'>
      ...
      <ModalBody>
        <div className='text-center'>
          {successIcon}
          <h1 className='mt-3 mb-2'>You're in!</h1>
          <p className='mb-2 font-medium-1'>What's your name?</p>
        </div>
        <div className='px-3 mb-2'>
          <Label className='form-label text-left' for='name'>
            Name
          </Label>
          <InputGroup id='name' className='input-group-merge mb-1'>
        ...
        <div className='mb-3 text-center'>
          <Button className="primary-btn" color='primary' onClick={() => handleSaveName(name)}>
            Save
          </Button>
        </div>
      </ModalBody>
